...
const RentProperty = () => {
  const {setSearchedLettingsProperties, searchedRentLocation, setSearchedRentLocation, searchedLettingsProperties} = useGlobalContext();
  const [filteredResult, setFilteredResult] = useState({location: searchedRentLocation || "", propertyType: "", minPrice: null, maxPrice: null, minBedrooms: null, maxBedrooms: null, bathrooms: null, features: "", isAvailableFrom: ""});
  ...
  const handleSearch = (e) =>{
    ...
    const onSubmit = async (data, e) => {
      ...
      try {
        ...
        if (queryString) {
          const fetchData = await axios.get(`${process.env.REACT_APP_API_URL}/lettings/advancedSearch?${queryString}`);
  ...
  return (
   ...
   <button type='submit' className='bg-yellow-600 text-white rounded-[0.5vw] hover:bg-yellow-500 text-[2vw] xl:text-[1.5vw] lg:text-[1.5vw] md:text-[1.5vw]  p-[0.7vw] absolute right-[14vw] xl:right-[15vw] lg:right-[15vw] md:right-[15vw] xl:top-0 lg:top-0 md:top-0 top-0.5vw'><Icon icon="bi:search" className='text-[3vw] xl:text-[1.5vw] lg:text-[1.5vw] md:text-[1.5vw]  font-medium text-white' /></button>
    ...
    {
      searchedLettingsProperties?.length > 0 ? <PropertySearch searchedProperties={searchedLettingsProperties} /> : <p className='text-2vw font-medium text-blue-950 text-center mt-2vw w-full'>nothing found. Search your desired property</p>
    }
    </form>
  )
}
